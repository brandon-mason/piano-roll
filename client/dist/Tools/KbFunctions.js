...
function KbFunctions(props) {
    ...
    function octaveUp() {
        if (props.selectorsRef.current) {
            ...
            //     let newOctave = parseInt(props.selectorsRef.current.children[i].nodeValue!) + 1;
            //     if(newOctave < props.octaveMinMax[1]) props.soundDispatch({type: 'octave', octave: newOctave})
            //   }
            // }
            var octaveSel = props.selectorsRef.current.children.namedItem('octave-selector');
            var newOctave = parseInt(octaveSel?.nodeValue) + 1;
